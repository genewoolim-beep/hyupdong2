...
# 2026-08-08 부로 block_sort 의 **기본값이 켬**이라 이 줄은 이제 없어도 같다
# (실기 검증이 끝났다 — 오늘 여러 번 치우고 그 자리를 열었다). 끄고 싶을 때
# RELOCATE_BLOCKERS=0 으로 띄울 수 있도록 형태만 남겨 둔다.
#
# 켜면 목적지를 두 가지로 검사한 뒤에만 옮긴다 — 다른 블록과 겹치지 않는지(비전),
# 판 안인지(교시 상자). 프리구역을 먼저 다 보고, 거기 자리가 없을 때만 구역 안도
# 쓴다(RELOCATE_ANYWHERE, 기본 켬). 그리고 치울 블록이 **어차피 이 계획에서
# 옮길 색**이면 임시 자리로 치우지 않고 곧장 제 구역에 놓는다.
