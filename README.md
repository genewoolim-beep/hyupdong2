...
**자기 자신을 옆 블록으로 세지 않게** `SELF_R`(30mm) 안의 검출은 버린다.
35mm 블록 둘은 중심이 35mm 보다 가까울 수 없으니, 그보다 가깝게 잡힌 것은 같은
블록을 다시 본 것이다(검출 흔들림). 이 값을 10mm 로 두었더니 흔들림이 그것을 넘는
순간 자기 자신이 이웃이 되어 **틈이 음수**가 되고, 두 축 다 막힌 것으로 보여
멀쩡한 블록을 거부했다(실측 2026-08-07).
...
```bash
python3 block_sort/test_avoid.py    # 로봇 없이 기하 검증 (6항목)
```
