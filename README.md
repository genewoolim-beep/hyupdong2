...
양쪽 좁으면: 양쪽 다 좁습니다 (틈 14mm < 손가락 27mm) — **집지 않습니다.**
```
...
**이웃의 기울기를 함께 본다** (2026-08-10 추가). 정사각형은 φ 만큼 돌면 그 방향
폭이 `35(|cos φ| + |sin φ|)` 로 커진다 — 반폭으로 17.5 → 최대 **24.7mm**, 즉 **7.2mm**
차이다. 이걸 안 보면 판정이 그만큼 낙관적이 된다: 실측 2026-08-10 에 목표
노란색(547,-74) 파지축 127° 에서 주황색(503,-21) 의 틈을 **34mm** 로 보고 "충분" 판정해
그대로 내려갔는데 **그리퍼가 그 주황색에 부딪혀 컨트롤러가 보호정지**했다. 그 판의
블록 기울기는 12·14·15·16·20·23·24·53·88·89° 로 축과 나란하지 않다.

검출은 블록마다 각도를 함께 주는데 `neighbors_xy` 가 그것을 버리고 있었다. 지금은
`(x, y, 색, 기울기)` 로 내보내 `block_geom.nb_half` 가 반폭을 기울기로 계산한다.
목표 자신은 손목을 제 면에 맞춰 내려가므로(`ALIGN_TO_BLOCK`) 보정이 필요 없다 —
**이웃뿐이다.** 각도를 안 넘기면 예전 값 그대로라 기존 호출부는 동작이 안 바뀐다.
...
**사방이 막히면 이웃을 치우고 집는다**(`RELOCATE_BLOCKERS`, **기본 켜짐** —
2026-08-08 실기 검증 완료). 어느 이웃이 막는지 찾아, 틈이 벌어질
만큼만 축 방향으로 밀어낸다. 목적지는 **두 가지를 다 본다** — 다른 블록과
겹치지 않는지(비전), 그리고 **판 안인지**. 판 경계는 조종 쪽 교시 상자를 빌린다
(`teach_box.py` → `teleop_box.env`, 같은 로봇·같은 판을 재려고 이미 실측한
유일한 값이다). 여유를 물린 `TX_MIN` 이 아니라 **교시 원본**을 쓴다 — 그 여유는
속도 지령이 미끄러지는 것을 위한 조종 쪽 몫이라, 여기 쓰면 인간구역 y −229 가
판 밖으로 판정된다.

**복제는 막는 색을 먼저 옮긴다**(`REORDER_FOR_BLOCKING`, 기본 켜짐). 뒤 순번 색이
앞 순번 색을 막고 있으면 임시 자리로 치웠다가 나중에 또 옮기는 두 번 이동이 된다 —
순서만 바꾸면 한 번에 끝난다. 이미 하기로 한 이동의 순서만 바꾸는 것이라 새 위험이 없다.

**자기 자신을 옆 블록으로 세지 않게** `SELF_R`(30mm) 안의 검출은 버린다.
**단, 빈 자리를 검사할 때는 `self_r=0` 을 준다** — 그 자리엔 자기 자신이 없으므로
기본값을 쓰면 30mm 안의 진짜 블록을 자기 자신으로 착각해 "비었다" 고 답하고,
그 위에 블록을 내려놓게 된다(이웃 치우기의 목적지 검사).
...
### 사방이 막히면 이웃을 치운다 (`RELOCATE_BLOCKERS`, 기본 켜짐)
...
대신 원래 목표 하나에 대해서는 `RELOCATE_MAX_TRIES`(기본 4)까지 — 손가락은 양쪽에서
내려오므로 한 축을 열려면 양쪽을 다 치워야 하고(2회), 검출 오차로 한 번 헛걸음하면
2회로는 모자란다(실측 2026-08-08: 십자 포위가 2회 상한에 걸려 포기했다).

**싼 것부터 다섯 단계로 푼다.**

| 단계 | 방법 | 비용 | 기본 |
|---|---|---|---|
| ① | 손목 90° 회전 (`best_axis`) | 공짜 — 정사각은 90° 대칭 | 켬 |
| ② | 순서 바꾸기 (`pick_order`, `choose_next`) | 공짜 — 로봇을 안 움직임 | 켬 |
| ③ | 계획 직행 (`relocate_to_zone`) | 막는 것이 어차피 옮길 색이면 곧장 제 구역으로 | 켬 |
| ④ | 끌기 (`slide_blocker`) | 잡은 채로 직각 60mm — 상승·이송·하강·상승 4번 절약 | 켬 |
| ⑤ | 들어 옮기기 (`relocate_one`) | 판 위 임시 자리(최대 230mm) — 배치가 흐트러진다 | **끔** |

다 실패하면 **직각 축의 방해물**로 전환한다 — `best_axis` 는 틈이 넓은 축을 고르지만
그 축의 방해물을 치울 수 있는지는 보지 않는다(실측 2026-08-08: 1mm 차이로 고른 축의
방해물이 x=648 이라 팔이 못 닿아 두 번 다 죽었다. 직각 축의 방해물은 x=597 로 멀쩡히
닿았다). 두 축은 어차피 같은 물림이니 **열 수 있는 쪽을 열면 된다.**

그래도 안 되면 **무엇을 시도했는지 남기고** 사람에게 넘긴다 — "시도했는데 실패한 것"
과 "후보가 없던 것"을 구별해서 찍는다.

**조정 손잡이**

| 환경변수 | 기본 | 뜻 |
|---|---|---|
| `RELOCATE_BLOCKERS` | `1` | 치우기 자체를 끄려면 `0` |
| `RELOCATE_MAX_TRIES` | `4` | 한 목표당 치우기 횟수 |
| `RELOCATE_CHAIN` | `1` | 갇힌 이웃의 방해물까지 파고들 단계. **더 올리지 말 것** — 단계마다 새로 집어 옮기는 블록이 늘고 판이 흐트러진다 |
| `RELOCATE_ANYWHERE` | `1` | 프리구역에 자리가 없을 때 **구역 안에도** 치울지. 인간구역에 떨어지면 본보기가 망가지므로 프리구역을 먼저 다 본다 |
| `SLIDE_BLOCKERS` | `1` | 끌기 사용 |
| `SLIDE_CHECK_PATH` | `0` | 끌 길을 볼지. 켜면 멀쩡한 방향까지 막혀 죽는 일이 잦았다 |
| `LIFT_RELOCATE` | `0` | 들어서 임시 자리로 옮기기 |
| `REACH_X_MAX` | `700` | 이 x 를 넘으면 **뒤로 미룬다**(거르는 게 아니다). 진짜 한계는 팔이 아니라 카메라 — 블록을 수직으로 보려면 팔이 핸드아이 오프셋만큼 더 바깥으로 나가야 한다(블록 x=635 → 관측 x=667 에서 실패). 로그 185건 분석에서 x 만 판별력이 있었다(중단 78% vs 수렴 5% at x≥630) |

**안전망.** 이번 파지에서 실패한 blocker 는 다시 고르지 않는다(`_failed_blockers`) —
실측 2026-08-08 에 같은 노란색(660,84)을 **12번 시도하며 129초**를 썼고, 그동안 20번의
검출에서 좌표가 1mm 도 안 변했다. 팔이 못 닿는 것(`in_reach`)도 후보에서 뺀다.

순수 기하 판단(`blocking_neighbor`, `relocate_step`, `slide_dests`, `axis_blockers`)은
로봇 없이 `test_relocate.py` 로 검증된다 — 실제로 집어 옮기는 동작과 목적지 충돌
확인은 로봇에서만 할 수 있다.
...
python3 voice_command.py selftest    # 마이크도 없이 정규화·해석만 검증
```

**새 LLM 프롬프트를 만들지 않았다.** 문장을 글로스로 정규화한 뒤
`sign_command.command_ready()` 를 그대로 부른다 — 그 함수는 글로스가 수어에서
왔는지 음성에서 왔는지 모른다. 색·구역·동작어를 사람이 부르는 표현은 몇 가지로
정해져 있어 정규식/사전 매칭이면 충분하고, LLM 왕복이 두 번 되는 것도 막는다.

**웨이크워드 조정** (`VOICE_WAKE_THRESHOLD`, 기본 **0.2**). 원래 0.4 였는데 그 값은
다른 마이크 기준이었다 — 노트북 내장 DMIC 는 조용한 방에서도 배경 음량이 4~5% 로
깔려 신뢰도가 그만큼 눌린다. 실측한 배경 신뢰도가 0.000~0.001 이라 0.2 로 낮춰도
오탐 여유가 200배 남고, 어차피 웨이크워드만으로는 로봇이 안 움직인다("실행" 확인).

**웨이크워드 모델도 바꿀 수 있다** (`VOICE_WAKE_MODEL`). 파일 경로 또는 openWakeWord
내장 이름(`alexa`, `hey_jarvis`, `hey_mycroft`, `hey_rhasspy`)을 받는다. 기본은
`hello_rokey_8332_32.tflite` 그대로다.

```bash
VOICE_WAKE_THRESHOLD=0.3 python3 voice_command.py watch
VOICE_WAKE_MODEL=alexa   python3 voice_command.py watch
```

같은 녹음으로 비교하면 `hello_rokey` 0.0013 / `alexa` 0.9312 / `hey_jarvis` 0.0092 로,
모델이 화자에 따라 전혀 안 잡히는 경우가 있다(특징 추출·마이크는 정상 — 안 그러면
`alexa` 도 못 터진다). 화면 안내문은 모델을 따라 자동으로 바뀐다.

**마이크가 안 잡히면** 시스템 기본 입력 장치부터 확인한다 — `voice_command.py` 는
`input_device_index` 를 안 넘기고 기본 장치를 그냥 연다.

```bash
pactl get-default-source          # 루프백(snd_aloop)이면 실제 마이크로 바꾼다
arecord -d 4 -f cd /tmp/m.wav && aplay /tmp/m.wav
```
