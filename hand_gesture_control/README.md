...
- xy(왼손, 검지 끝 기준): **십자선**. 화면 왼쪽 아래 십자 — 중앙 사각(데드존)이면
  정지, 어느 쪽으로 벗어나든 그 축으로 **일정 속도**(20mm/s). 얼마나 벗어났는지는
  속도를 안 바꾼다 — 속도가 변하지 않아야 예측 가능하다.
- z(오른손, 손바닥 기준): 화면 오른쪽 세로 게이지 안에서 손바닥 높이의 절대
  위치로 직접 정한다. 이동평균+데드밴드로 손떨림을 억제한다.
