...
| `REALSENSE_MIRROR` | `1` | 로봇 시점을 좌우로 뒤집는다. 조종자가 로봇을 **마주보고** 서기 때문 |
...
화면은 **반시계 90°(세로)로 돌리고 좌우로 뒤집어 보낸다**
(`REALSENSE_ROTATE`, `REALSENSE_MIRROR`). 뒤집는 이유는 조종자가 로봇을 마주보기
때문이다 — 내 오른쪽이 로봇 카메라에는 왼쪽으로 잡힌다. 뒤집어 두면 화면에서
오른쪽으로 보이는 것이 내 오른쪽이라 손을 어느 쪽으로 밀지 헷갈리지 않는다.
거울은 **돌린 뒤에** 걸고, **AR 글자는 뒤집지 않는다** — 거울 글자는 못 읽는다. 판을 내려다보는
