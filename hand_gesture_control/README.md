...
| `GESTURE_CROP` | `0.75` | 긴 변에서 가운데 이만큼만 남긴다. 이 웹캠의 최대는 1280x720(16:9)이라 너무 길다 |
...
카메라를 4:3(640x480)으로 열면 되지만 웹캠 토픽은 **수어 인식과 공유**한다 —
거기 해상도를 건드리면 학습된 인식기까지 영향을 받는다. 그래서 조종 쪽에서만
잘라낸다(`orient()`: 자르기 → 회전 → 거울).

```
GESTURE_CROP=1.0   1280x720 → 세로 720x1280  (0.56:1, 아주 길다)
GESTURE_CROP=0.75   960x720 → 세로  720x960  (0.75:1, 기본)
GESTURE_CROP=0.6    768x720 → 세로  720x768  (0.94:1, 거의 정사각)
```

덤으로 손이 화면에서 커져 인식에도 낫다. 대신 좌우 시야를 잃으므로 손을 넓게
벌리면 화면을 벗어난다 — 그러면 값을 키운다.
