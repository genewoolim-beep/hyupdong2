...
# ── 좌우 방향 (조종하는 사람이 어디 서 있는가) ────────────────────
# 이건 코드가 정할 수 없는 값이다. **로봇을 마주보고** 조종하면 내 오른쪽이
# 로봇의 왼쪽이라 좌우가 뒤집혀야 하고, 로봇과 **같은 쪽을 보고**(뒤나 옆에서)
# 조종하면 그대로여야 한다. 화면이 이미 거울(cv2.flip)이라 손을 오른쪽으로
# 옮기면 화면 표시도 오른쪽으로 가는데, 그 오른쪽이 base 의 어느 부호인지는
# 사람이 선 자리로 정해진다.
#
# 1 = 로봇을 마주보고 조종 (실사용 기본. 2026-08-07 에 좌우가 반대라고 확인됨)
# 0 = 로봇과 같은 방향을 보고 조종
#
# 앞뒤(x)는 건드리지 않는다. 마주보면 앞뒤도 뒤집히는 게 기하적으로 맞지만,
# 실사용에서 앞뒤는 맞는다고 확인됐다(2026-08-07). 몸을 돌려도 팔을 뻗는
# '앞' 은 판 쪽으로 유지하기 때문으로 보인다. 앞뒤까지 뒤집으려면
# TELEOP_FACE_ROBOT 을 늘리지 말고 그때 따로 상수를 둘 것 — 두 축을 한 스위치로
# 묶으면 한쪽만 고치고 싶을 때 못 고친다.
FACE_ROBOT = os.environ.get("TELEOP_FACE_ROBOT", "1") != "0"
Y_SIGN = -1.0 if FACE_ROBOT else 1.0
...
class RobotTeleop:
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"  로봇 조종 연결됨 — 시작 위치 "
                  ...
                  f"데드존 xy {DEADZONE:.2f} / z {Z_DEADZONE:.2f}")
    ...
    def update(self, vec, z_norm, gripper_open, hand_seen):
        ...
        z_norm       0~1. 게이지 안에서의 손 높이. **중앙은 0.5** — 가리키는
                     절대 높이가 아니라 xy 와 같은 방향 신호다.
        ...
        # 화면 위(v<0) = 로봇 앞(+x).
        # 좌우는 선 자리에 달렸다(Y_SIGN 주석). 마주보고 조종하는 기본값에서는
        # 화면 오른쪽(u>0) = 내 오른쪽 = 로봇의 왼쪽(+y) 이다.
        vx = -TELEOP_SPEED if v > DEADZONE else (TELEOP_SPEED if v < -DEADZONE else 0.0)
        # 부호를 곱하지 않고 갈라 쓴다. 0.0 에 -1 을 곱하면 -0.0 이 되어
        # 로그와 화면에 "-0" 으로 찍힌다 — 정지인데 뭔가 나가는 것처럼 보인다.
        if u > DEADZONE:
            vy = -TELEOP_SPEED * Y_SIGN
        elif u < -DEADZONE:
            vy = TELEOP_SPEED * Y_SIGN
        else:
            vy = 0.0
