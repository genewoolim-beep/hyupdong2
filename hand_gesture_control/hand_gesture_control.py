...
# 긴 변에서 가운데 이만큼만 남긴다 (1.0 = 자르지 않음, 기본).
#
# **기본은 자르지 않는 것이다.** 화면이 좌우로 긴 것은 이 웹캠의 최대가
# 1280x720(16:9)이라 그렇고, 잘라서 줄이면 그만큼 시야를 잃는다 — 손을 넓게
# 벌리면 화면을 벗어난다. 대시보드 쪽은 **틀이 영상 비율을 따라가게** 고쳤으므로
# 안 잘라도 검은 여백이 생기지 않는다(style.css 의 gesture-cam-panel).
#
# 그래도 좁히고 싶으면 값을 줄인다. 손이 커져 인식에는 낫다.
#   1.0   1280x720 → 세로 720x1280 (0.56:1)
#   0.75   960x720 → 세로  720x960 (0.75:1, 4:3)
#   0.6    768x720 → 세로  720x768 (0.94:1)
GESTURE_CROP = float(os.environ.get("GESTURE_CROP", 1.0))
