...
왼손: xy 제어. 손목의 화면상 위치를 그대로 정규화해서 쓴다. 팔 각도를 거치지
  않으므로 사람이 서 있는 위치가 바뀌어도 "손이 화면 어디에 있는가"가 곧바로
  좌표가 된다. 손이 화면 전체를 오가야 하는 부담을 줄이기 위해, 실제로 이
  좌표를 뽑아내는 감지/매핑 범위는 화면 왼쪽에 놓인 가로*세로 절반(면적 1/4)
  박스(ROI_X0~X1, ROI_Y0~Y1)로 제한한다. 손이 이 박스 안에서만 움직여도
  전체 0~1 xy 범위를 커버한다.

오른손: z 축과 그리퍼 제어. MediaPipe GestureRecognizer 의 손 모양 실시간
  분류로 제어한다. thumbs up/down 특유의 손떨림이 xy 를 왜곡하지 않도록 z/그립
  조작을 xy 를 담당하는 손과 물리적으로 분리했다.
  Thumb_Up   : 누르고 있는 동안 z 상승
  Thumb_Down : 누르고 있는 동안 z 하강
  Open_Palm  : 그리퍼 열기
  Closed_Fist: 그리퍼 닫기
...
키
  Q 종료          S 시간평균 on/off
  [ ] 게인 조절    SPACE 중심 재설정 (지금 왼손 위치를 화면 중앙으로)
  R 초기화        F 전체화면
"""
...
GAIN0 = 1.6               # 정규화 오프셋 -> 화면 비율 (손 이동폭이 화면 전체를 못 덮으므로 확대)
SMOOTH_N = 5
SCREEN_ASPECT = 16 / 9

Z_RATE = 0.35             # 초당 z 변화량 (Thumb_Up/Down 유지 시), 0~1 범위
Z0 = 0.5
...
# xy 감지/제어 영역: 화면(카메라 프레임) 왼쪽에 놓인 가로*세로 절반 크기 박스
# (면적 기준 화면의 1/4). 왼손 손목이 이 박스 안에서 움직이는 것만으로
# 전체 0~1 xy 범위를 커버하도록 매핑한다. 값은 프레임 비율(0~1) 기준.
ROI_X0, ROI_X1 = 0.0, 0.5
ROI_Y0, ROI_Y1 = 0.25, 0.75

KEY_HELP = [
    ("Q", "종료"),
    ("SPACE", "중심 재설정 (현재 왼손 위치 -> 화면 중앙)"),
    ("[ / ]", "게인 감소 / 증가"),
    ("S", "시간평균 on/off"),
    ("R", "전체 초기화"),
    ("F", "전체화면 전환"),
...
class HandController:
    def __init__(self):
        ...
        self.smooth = True
        self.gain = GAIN0
        self.center = np.zeros(2)     # SPACE 로 잡는 기준 오프셋 (왼손 기준)
        self.z = Z0
    ...
    def process(self, frame, dt):
        ...
        xy_hand      = (화면비율 xy, 랜드마크 픽셀 배열, ROI 정규화 좌표) 또는 None
        ...
        for i, lm in enumerate(res.hand_landmarks):
            ...
            if label == "Right":  # 실제 왼손 (핸드니스 반전, 모듈 docstring 참고)
                ...
                # 손목 좌표를 화면 왼쪽 ROI 박스(면적 1/4) 기준으로 정규화한다.
                nx = (lm[WRIST].x - ROI_X0) / (ROI_X1 - ROI_X0)
                ny = (lm[WRIST].y - ROI_Y0) / (ROI_Y1 - ROI_Y0)
                v = np.array([nx, ny]) - 0.5     # -0.5~0.5 로 중심화
                xy = np.array([0.5, 0.5]) + (v - self.center) * self.gain
                xy_hand = xy, px, v
                ...
                if self.gesture == "Thumb_Up":
                    self.z = min(1.0, self.z + Z_RATE * dt)
                elif self.gesture == "Thumb_Down":
                    self.z = max(0.0, self.z - Z_RATE * dt)
...
def draw_hand(img, px):
    ...
    cv2.circle(img, tuple(px[WRIST].astype(int)), 8, (0, 255, 120), 2, cv2.LINE_AA)
...
def draw_roi(frame):
    ...
    cv2.putText(frame, "xy ROI (left hand)", (x0 + 6, y0 + 22),
...
def run_session():
    ...
    h, w = frame.shape[:2]
    print(f"카메라 {w}x{h}")
    print("손을 카메라에 보이세요. SPACE = 지금 손 위치를 화면 중앙으로 맞춤")
    if USE_ADMIN:
    ...
    while True:
        ...
        if xy_hand is not None:
            draw_hand(frame, xy_hand[1])
        if gesture_hand is not None:
            draw_hand(frame, gesture_hand)

        pt = P.update_xy(xy_hand[0] if xy_hand else None)
        ...
        if pt is not None:
            # 화면 밖으로 나가도 가장자리에 붙여서 보여준다
            cx = int(np.clip(pt[0], 0.0, 1.0) * (SW - 1))
            cy = int(np.clip(pt[1], 0.0, 1.0) * (SH - 1))
            edge = not (0.0 <= pt[0] <= 1.0 and 0.0 <= pt[1] <= 1.0)
            cols = [(0, 70, 30), (0, 150, 65), (70, 255, 150)] if not edge \
                else [(0, 50, 70), (0, 110, 150), (80, 200, 255)]
            for rad, col in zip((34, 21, 11), cols):
        ...
        if k == ord('q'):
            ...
            P.center = np.zeros(2); P.gain = GAIN0; P.hist.clear()
            P.z = Z0; P.gripper = GRIPPER_OPEN
            print("초기화")
        elif k == 32 and xy_hand is not None:
            P.center = xy_hand[2].copy()      # 지금 왼손 위치를 중앙으로
            P.hist.clear()
            print(f"중심 재설정: {P.center.round(3)}")
        elif k == ord('f'):
            full = not full
