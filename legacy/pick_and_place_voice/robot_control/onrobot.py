...
try:  # pymodbus 2.x
    ...
    _LEGACY_PYMODBUS = True
    SLAVE = {"unit": 65}
except ImportError:  # pymodbus 3.x
    from pymodbus.client import ModbusTcpClient as ModbusClient
    _LEGACY_PYMODBUS = False
    SLAVE = {"slave": 65}
...
class RG:
    ...
    def __init__(self, gripper, ip, port):
        if _LEGACY_PYMODBUS:
            self.client = ModbusClient(
                ip, port=int(port), stopbits=1, bytesize=8, parity="E",
                baudrate=115200, timeout=1
            )
        else:
            self.client = ModbusClient(ip, port=int(port), timeout=1)
    ...
    def _request(self, method_name, **kwargs):
        """Sends a Modbus request, reconnecting once if the link went stale.

        The toolchanger drops idle TCP connections without notice, so a long
        running program hits a half-open socket on its first command.
        """
        for attempt in (1, 2):
            try:
                ...
                if attempt == 2:
                    raise
                print("Modbus link stale, reconnecting.")
    ...
    def get_fingertip_offset(self):
        ...
        result = self._request(
            "read_holding_registers", address=258, count=1, **SLAVE)
    ...
    def get_width(self):
        ...
        """
        result = self._request(
            "read_holding_registers", address=267, count=1, **SLAVE)
    ...
    def get_status(self):
        ...
        result = self._request(
            "read_holding_registers", address=268, count=1, **SLAVE)
    ...
    def get_width_with_offset(self):
        ...
        """
        result = self._request(
            "read_holding_registers", address=275, count=1, **SLAVE)
        width_mm = result.registers[0] / 10.0
        return width_mm
    ...
    def set_control_mode(self, command):
        ...
        """
        result = self._request(
            "write_register", address=2, value=command, **SLAVE)
    ...
    def set_target_force(self, force_val):
        ...
        """
        result = self._request(
            "write_register", address=0, value=force_val, **SLAVE)
    ...
    def set_target_width(self, width_val):
        ...
        """
        result = self._request(
            "write_register", address=1, value=width_val, **SLAVE)
    ...
    def close_gripper(self, force_val=400):
        ...
        result = self._request(
            "write_registers", address=0, values=params, **SLAVE)
