...
YOLO_MODEL_FILENAME = "fruit_best.pt"
YOLO_CLASS_NAME_JSON = "fruit_class_name.json"
...
class YoloModel:
    def __init__(self):
        ...
        with open(YOLO_JSON_PATH, "r", encoding="utf-8") as file:
            ...
            # 클래스명이 'Apple'처럼 대문자로 시작해서, 소문자로 들어와도 찾히게 한다
            self.reversed_class_dict = {v.lower(): int(k) for k, v in class_dict.items()}
    ...
    def get_best_detection(self, img_node, target):
        ...
        label_id = self.reversed_class_dict.get(target.strip().lower())
        if label_id is None:
            print(f"'{target}' is not a known class. known: {sorted(self.reversed_class_dict)}")
            return None, None
